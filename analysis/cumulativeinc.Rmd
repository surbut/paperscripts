...
fit <- survfit(Surv((phenos.CAD_censor_age), phenos.has_CAD) ~agecat, data=df)
...
ppce<-ggplot(a, aes(x=PCE.percentile, y=Age.of.Event, fill=PCE.percentile,col=PCE.percentile)) + 
  ...
  geom_errorbar(aes(ymin=Age.of.Event-se, ymax=Age.of.Event+se), width=.02,
                position=position_dodge(0.05))+ylim(50,71)+theme_classic()+stat_smooth()+ylab("Mean Age of Event")+xlab("Pooled Cohort Equation Decile")
...
```{r gyoungprs}
ggsave(g_young$plot,filename = "output/gyoung.tiff",dpi = 300,width = 10,height = 4)
plots[[4]]=g_young
write.table(g_young$data.survtable,file = "output/cumulativeinc55.txt")
...
g_mid
plots[[5]]=g_mid
ggsave(g_mid$plot,filename = "output/gmid.tiff",dpi = 300,width = 10,height = 4)
write.table(g_mid$data.survtable,file = "output/cumulativeinc55-65.txt")
```
...
g_old
plots[[6]]=g_old
ggsave(g_old$plot,filename = "output/gold.tiff",dpi = 300,width = 10,height = 4)
write.table(g_old$data.survtable,file = "output/cumulativeinc65.txt")
```
