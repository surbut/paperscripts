...
fitprs <- survfit(Surv(phenos.CAD_censor_age, phenos.has_CAD) ~prscat, data=df)

g_prs= ggsurvplot(fitprs,
...
```{r, gallpce}
#g_pce
ggsave(g_pce$plot,filename="Figs/gpce.tiff",dpi=300)
...
agefit <- survfit(Surv((phenos.CAD_censor_age-phenos.enrollment), phenos.has_CAD) ~agecat, data=df)

genroll=ggsurvplot(agefit, data = df,
           pval = TRUE, conf.int = TRUE,
           risk.table = F, 
           #risk.table.col = "strata",
           palette="futurama",
...
pprs<-ggplot(a, aes(x=PRS.percentile, y=Age.of.Event, fill=PRS.percentile,col=PRS.percentile)) + 
  ...
  geom_errorbar(aes(ymin=Age.of.Event-se, ymax=Age.of.Event+se),width=0.01,
                )+ylim(50,71)+theme_classic()+stat_smooth()+ylab("Mean Age of Event")+xlab("Polygenic Risk Decile")
p=a
...
plot<-ggplot(r, aes(x=Decile, y=Age.of.Event,group=Score, fill=Score,col=Score)) + 
  geom_point()+
  geom_errorbar(aes(ymin=Age.of.Event-se, ymax=Age.of.Event+se), width=.02)+ylim(50,71)+theme_classic()+stat_smooth()+ylab("Mean Age of Event,years")+xlab("Decile")+scale_fill_manual(values=c("cornflowerblue","red"))+theme_classic(base_size = 15)+scale_color_manual(values=c("cornflowerblue","red"))+scale_x_continuous(breaks = seq(0, 1, 0.10))+labs(col="Risk Stratification")+ guides(fill = "none")
...
dyoung$pce.r=rank(dyoung$pce.age)/length(dyoung$pce.age)
dyoung$pce.cat=cut(dyoung$pce.r,breaks=c(0,0.20,0.80,1))
dyoung$Interaction=interaction(dyoung$prscat,dyoung$ascvdcat_all)
...
colorRampPalette(c("#FEE6CE", "#FDAE6B", "#E6550D"))(3)),
          ...
          legend.labs = c("PRS low:PCE Low",
                          "PRS Intermediate:PCE Low",
                          "PRS High:PCE Low",
                          "PRS low:PCE Intermediate",
                          "PRS Intermediate:PCE Intermediate",
                          "PRS High:PCE Intermediate",
                          "PRS low:PCE High",
                          "PRS Intermediate:PCE High",
                          "PRS High:PCE High"))
###

dmid=df[df$phenos.enrollment>55&df$phenos.enrollment<65,]
dmid$pce.age=scale(dmid$ascvd_10y_accaha_all)# rescale for age
...
gg=ggarrange(g_young$plot,g_mid$plot,g_old$plot,nrow=3,ncol=1,labels=c("A.","B.","C"),common.legend = T)
ggsave(gg,dpi=300,filename = "Figs/interaction_time_bytimesinceenro.tiff")
```
...
#fit <- survfit(Surv((phenos.CAD_censor_age-phenos.enrollment),phenos.has_CAD)~Interaction, data=dyoung)
fit <- survfit(Surv(phenos.CAD_censor_age,phenos.has_CAD)~pce.cat, data=dyoung)


g_youngpce=ggsurvplot(fit,
           pval = TRUE, censor=FALSE,
           #conf.int = TRUE,
           ##risk.table = F, 
           ##risk.table.col = "strata",
           fun="cumhaz",
           #xlim = c(0, 15), 
           xlim = c(40, 70),break.x.by=5,
            #xlab = "Time since Enrollment",
           xlab = "Time of Event (Age)",
           ylab = "Cumulative Hazard",
           ylim=c(0,0.15),
           title = "Enrollment <55",
          legend.title = "PCE Rank",
          ggtheme = theme_classic2(base_size=12, base_family = "Arial"),
           font.family = "Arial",
          palette=c("#3182BD","#31A354","#E6550D"),
           legend.labs = c("PCE <20%", "PCE 20-80%", "PCE >80%"))


## with PRS

fit <- survfit(Surv(phenos.CAD_censor_age,phenos.has_CAD)~prscat, data=dyoung)


g_youngprs=ggsurvplot(fit,
           pval = TRUE, censor=FALSE,
           #conf.int = TRUE,
           ##risk.table = F, 
           ##risk.table.col = "strata",
...
dmid$pce.cat=cut(dmid$pce.r,breaks=c(0,0.20,0.80,1))
dmid$Interaction=interaction(dmid$prscat,dmid$pce.cat)
#dmid$Interaction=interaction(dmid$prscat,dmid$ascvdcat_all)

fit <- survfit(Surv(phenos.CAD_censor_age,phenos.has_CAD)~pce.cat, data=dmid)

g_midpce=ggsurvplot(fit,break.x.by=5,
...
fit <- survfit(Surv(phenos.CAD_censor_age,phenos.has_CAD)~prscat, data=dmid)


g_midprs=ggsurvplot(fit,
           pval = TRUE, censor=FALSE,
           #conf.int = TRUE,
           ##risk.table = F, 
           ##risk.table.col = "strata",
...
g_mid=ggsurvplot(fit,
          ...
          ggtheme = theme_classic2(base_size=12, base_family = "Arial"),
           #font.family = "Arial",
                 font.family = "Arial",break.x.by = 5,
palette=c(colorRampPalette(c("#DEEBF7", "#9ECAE1", "#3182BD"))(3),
colorRampPalette(c("#E5F5E0", "#A1D99B", "#31A354"))(3),
colorRampPalette(c("#FEE6CE", "#FDAE6B", "#E6550D"))(3)),
...
dold$pce.r=rank(dold$pce.age)/length(dold$pce.age)
dold$pce.cat=cut(dold$pce.r,breaks=c(0,0.20,0.80,1))
dold$Interaction=interaction(dold$pce.cat,dold$prscat)
#dold$Interaction=interaction(dold$prscat,dold$ascvdcat_all)

fit <- survfit(Surv(phenos.CAD_censor_age,phenos.has_CAD)~pce.cat, data=dold)


g_oldpce=ggsurvplot(fit,
           pval = TRUE, 
           conf.int = TRUE,break.x.by=5,
           censor=FALSE,
           ##risk.table = F, 
           ##risk.table.col = "strata",
           fun="cumhaz",
...
fit <- survfit(Surv(phenos.CAD_censor_age,phenos.has_CAD)~, data=dold)


g_oldaha=ggsurvplot(fit,
           pval = TRUE, 
           conf.int = TRUE,break.x.by=5,
           censor=FALSE,
           ##risk.table = F, 
...
fit <- survfit(Surv(phenos.CAD_censor_age,phenos.has_CAD)~prs.cat, data=dold)


g_oldprs=ggsurvplot(fit,
           pval = TRUE, censor=FALSE,
           #conf.int = TRUE,
           ##risk.table = F, 
           ##risk.table.col = "strata",
...
colorRampPalette(c("#E5F5E0", "#A1D99B", "#31A354"))(3),
colorRampPalette(c("#FEE6CE", "#FDAE6B", "#E6550D"))(3)),
break.x.by = 5,legend.labs = c("PRS low:PCE low", "PRS int:PCE low", "PRS high:PCE low",
...
gg1=ggarrange(g_youngprs$plot,g_midprs$plot,g_oldprs$plot,nrow=1,ncol=3,labels=c("A.","B.","C."),common.legend = T)



gg2=ggarrange(g_young$plot,g_mid$plot,g_old$plot,nrow=1,ncol=3,labels=c("D","F","G"),common.legend = T)


gg3=ggarrange(g_youngpce$plot,g_midpce$plot,g_oldpce$plot,nrow=1,ncol=3,labels=c("D","F","G"),common.legend = T)

ggsave(gg1,dpi=300,filename = "Figs/interaction_time_byage_prs.tiff",height=10,width = 10)
ggsave(gg2,dpi=300,filename = "Figs/interaction_time_byage.tiff")
ggsave(gg3,dpi=300,filename = "Figs/interaction_time_byage_pce.tiff",height=10,width = 10)
```
...
# Combine plots
p_combined <- ggdraw() +
draw_plot(p$plot, 0, 0, 1, 1) +
draw_plot(p_inset$plot, 0.50, 0.50, 0.5, 0.5)
# Move legend to bottom
p_combined_young <- p_combined +
theme(legend.position = "bottom",
legend.margin = margin(t = 1, r = 1, b = 1, l = 1, unit = "lines"))


ggsave(p_combined_young,file="pcombinedyoung.tiff",dpi=300)
### mid
...
p_combined <- ggdraw() +
draw_plot(p$plot, 0, 0, 1, 1) +
draw_plot(p_inset$plot, 0.5, 0.50,0.50,0.50)
# Move legend to bottom
p_combined_mid <- p_combined +
theme(legend.position = "bottom",
legend.margin = margin(t = 1, r = 1, b = 1, l = 1, unit = "lines"))

ggsave(p_combined_mid,file="pcombinedmid.tiff",dpi=300)
### mid
...
draw_plot(p_inset$plot, 0.5, 0.50,0.50,0.50)
# Move legend to bottom
p_combined_old <- p_combined +
theme(legend.position = "bottom",
legend.margin = margin(t = 1, r = 1, b = 1, l = 1, unit = "lines"))


ggsave(p_combined_old,file="pcombinedold.tiff",dpi=300)
### mid


ggsave(grid.arrange(p_combined_young,p_combined_mid,p_combined_old,nrow=1,ncol=3,widths=c(6,6,6),heights=4),file="alltogether.tiff",dpi=300)

nrow=1,ncol=3,height=4,width=12)
